...
// ============================================================
// VERSICHERUNG — dedicated sub-tabs (Zuständige Versicherung + Vertrag)
// with detail view showing Details / Versicherungsschein / Korrespondenz /
// Kündigung, tailored to insurance contracts.
// ============================================================
...
class _VersicherungTab extends StatefulWidget {
  final ApiService apiService;
  final int userId;
  final List<Map<String, dynamic>> vertraege;
  final Future<void> Function() onChanged;
  const _VersicherungTab({
    required this.apiService,
  ...
  @override
  State<_VersicherungTab> createState() => _VersicherungTabState();
}

class _VersicherungTabState extends State<_VersicherungTab> {
  ...
  List<Map<String, dynamic>> get _zustaendige {
    final ids = <int>{};
    for (final v in widget.vertraege) {
      final id = int.tryParse(v['versicherung_id']?.toString() ?? '');
      if (id != null) ids.add(id);
    }
    final all = _byId;
    return ids.map((id) => all[id]).whereType<Map<String, dynamic>>().toList();
  }
          ...
          child: TabBar(
            ...
            tabs: [
              Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.circle, size: 8, color: _zustaendige.isNotEmpty ? Colors.green : Colors.red),
                const SizedBox(width: 4),
                const Icon(Icons.shield, size: 14),
                const SizedBox(width: 4),
                Text('Zuständige Versicherung (${_zustaendige.length})'),
              ])),
              Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.circle, size: 8, color: widget.vertraege.isNotEmpty ? Colors.green : Colors.red),
                const SizedBox(width: 4),
                const Icon(Icons.description, size: 14),
                const SizedBox(width: 4),
                Text('Vertrag (${widget.vertraege.length})'),
              ])),
            ],
          ),
        ),
        Expanded(child: TabBarView(children: [
          _buildZustaendigeTab(),
          _buildVertragTab(),
        ])),
      ]),
    );
        ...
        if (_zustaendige.isEmpty)
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 8),
              Text('Noch keine Zuständige Versicherung',
                style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
              const SizedBox(height: 4),
              Text('Legen Sie unter "Vertrag" einen Vertrag an — die Versicherung erscheint automatisch hier.',
          ...
          ..._zustaendige.map((v) {
            final contractsHere = widget.vertraege
                .where((c) => c['versicherung_id']?.toString() == v['id']?.toString()).toList();
            return Container(
              width: double.infinity,
              margin: const EdgeInsets.only(bottom: 12),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Icon(Icons.shield, size: 22, color: Colors.green.shade700),
                  const SizedBox(width: 8),
                  ...
                  Container(
                    ...
                    child: Text('${contractsHere.length} Vertrag${contractsHere.length == 1 ? '' : 'e'}',
  ...
  Widget _infoRow(IconData icon, String text) {
    return Padding(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: Text(text, style: TextStyle(fontSize: 12, color: Colors.green.shade800))),
      ]),
    );
  }

  Widget _buildVertragTab() {
    final byId = _byId;
    final aktive = widget.vertraege.where((v) => v['is_active'] == 1 || v['is_active'] == true || v['is_active'] == '1').toList();
    final inaktive = widget.vertraege.where((v) => !(v['is_active'] == 1 || v['is_active'] == true || v['is_active'] == '1')).toList();
    return Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(16, 12, 16, 8), child: Row(children: [
        Icon(Icons.description, size: 20, color: Colors.green.shade700),
        const SizedBox(width: 8),
        Expanded(child: Text('Verträge (${widget.vertraege.length})',
          style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.green.shade700))),
        ElevatedButton.icon(
          onPressed: _addVertragDialog,
          icon: const Icon(Icons.add, size: 16),
          label: const Text('Neuer Vertrag', style: TextStyle(fontSize: 12)),
      ...
      Expanded(child: widget.vertraege.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 8),
              Text('Keine Verträge', style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
            ]))
  ...
  Future<void> _addVertragDialog({Map<String, dynamic>? existing}) async {
    final id = int.tryParse(existing?['id']?.toString() ?? '');
    int? selVersId = int.tryParse(existing?['versicherung_id']?.toString() ?? '');
    String sparte = existing?['tarif']?.toString() ?? 'haftpflicht';
          ...
          Container(
            ...
            child: Row(children: [
              ...
              OutlinedButton.icon(
                ...
                onPressed: () async {
                  final picked = await _showVersicherungSearch(ctx2);
                  if (picked != null) {
  ...
  Future<Map<String, dynamic>?> _showVersicherungSearch(BuildContext hostCtx) async {
    String q = '';
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx2, setD) {
        ...
        return AlertDialog(
          ...
          title: Row(children: [
            ...
            const SizedBox(width: 8),
            const Text('Versicherung auswählen'),
          ]),
          content: SizedBox(
    ...
    return DefaultTabController(
      ...
      child: Column(children: [
        ...
        Expanded(child: TabBarView(children: [
          ...
          _DokTab(apiService: apiService, vertragId: vertragId, kategorie: 'versicherungsschein', label: 'Versicherungsschein'),
          _KorrTab(apiService: apiService, vertragId: vertragId),
          _DokTab(apiService: apiService, vertragId: vertragId, kategorie: 'kuendigung', label: 'Kündigung'),
        ])),
      ]),
    );
  }
