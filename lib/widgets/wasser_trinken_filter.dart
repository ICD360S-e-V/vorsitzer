      ...
      child: SizedBox(width: 550, child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(p['name']?.toString() ?? '', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: F.h(color, 800))),
            Text(p['marke']?.toString() ?? '', style: TextStyle(fontSize: 13, color: F.h(Colors.grey, 600))),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: F.h(color, 100), borderRadius: BorderRadius.circular(20)),
            child: Text(p['preis']?.toString() ?? '', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: F.h(color, 800)))),
          const SizedBox(width: 8),
          IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(ctx)),
        ]),
        const SizedBox(height: 12),
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: F.h(color, 50), borderRadius: BorderRadius.circular(8)),
          child: Text(p['typ']?.toString() ?? '', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: F.h(color, 700)))),
        const SizedBox(height: 12),
            ...
            return Card(margin: const EdgeInsets.only(bottom: 10), elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              child: InkWell(onTap: () => _openDetail(p), borderRadius: BorderRadius.circular(12),
                child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
                  Container(width: 50, height: 50, decoration: BoxDecoration(color: F.h(color, 100), borderRadius: BorderRadius.circular(12)),
                    child: Icon(_getIcon(p['typ']?.toString() ?? ''), size: 28, color: F.h(color, 700))),
                  const SizedBox(width: 14),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(p['name']?.toString() ?? '', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: F.h(color, 800))),
                    Text(p['marke']?.toString() ?? '', style: TextStyle(fontSize: 11, color: F.h(Colors.grey, 600))),
                    const SizedBox(height: 4),
                    Row(children: [
                      Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: F.h(color, 50), borderRadius: BorderRadius.circular(6)),
                        child: Text(p['typ']?.toString() ?? '', style: TextStyle(fontSize: 9, color: F.h(color, 700), fontWeight: FontWeight.w600))),
                      if (bewertung.isNotEmpty) ...[const SizedBox(width: 8), Text(bewertung, style: const TextStyle(fontSize: 11))],
                    ]),
                  ])),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(p['preis']?.toString() ?? '', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: F.h(color, 800))),
                    const SizedBox(height: 4),
