  ...
  String _antragArtText(String art, String behoerdeType) {
    if (behoerdeType == 'bundesagentur') {
      ...
      const map = {
        ...
        'altersrente_langjaehrig': 'Altersrente fuer langjaehrig Versicherte (35 J.)',
        'altersrente_besonders_langjaehrig': 'Altersrente fuer besonders langjaehrig Versicherte (45 J.)',
        'altersrente_schwerbehindert': 'Altersrente fuer schwerbehinderte Menschen',
        'emr_voll': 'Erwerbsminderungsrente (voll)',
        'emr_teil': 'Erwerbsminderungsrente (teilweise)',
        'erziehungsrente': 'Erziehungsrente (§ 47 SGB VI)',
        ...
        'reha': 'Medizinische Rehabilitation',
        'lta': 'Leistungen zur Teilhabe am Arbeitsleben (LTA)',
        'uebergangsgeld': 'Uebergangsgeld',
        'reha_nachsorge': 'Reha-Nachsorge (IRENA)',
        'beitragserstattung': 'Beitragserstattung (§ 210 SGB VI)',
        'rentensplitting': 'Rentensplitting unter Ehegatten',
        'zuschuss_kv': 'Zuschuss zur Krankenversicherung',
...
class _AntragBescheideTabState extends State<_AntragBescheideTab> {
  Map<int, List<Map<String, dynamic>>> _byJahr = {};
  bool _loading = true;
  String? _error;
    ...
    try {
      ...
      if (r['success'] == true) {
        ...
        for (final it in items) {
          ...
          if (j > 0) {
            map.putIfAbsent(j, () => <Map<String, dynamic>>[]).add(it);
  ...
  Future<void> _upload(int jahr) async {
    ...
    setState(() { _busyJahr = true; _activeJahr = jahr; _doneCount = 0; _totalCount = files.length; });
    final errors = <String>[];
    ...
    if (!mounted) return;
    setState(() { _busyJahr = false; _activeJahr = null; });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(errors.isEmpty
        ? '$_doneCount/$_totalCount Datei(en) für $jahr hochgeladen'
  ...
  Future<void> _delete(int id, String filename) async {
    final ok = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Datei löschen?'),
      content: Text(filename, style: const TextStyle(fontSize: 12)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Abbrechen')),
              ...
              if (hasFiles) ...files.map((d) => Container(
                ...
                child: Row(children: [
                  ...
                  IconButton(
                    ...
                    tooltip: 'Löschen',
                    onPressed: () => _delete(d['id'] as int, d['filename']?.toString() ?? '—'),
                    visualDensity: VisualDensity.compact,
                    padding: const EdgeInsets.all(2),
                    constraints: const BoxConstraints(),
                  ),
