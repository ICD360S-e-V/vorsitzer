...
class _DeoTabState extends State<DeoTab> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(length: 1, child: Column(children: [
      TabBar(labelColor: F.h(Colors.purple, 800), unselectedLabelColor: F.h(Colors.grey, 500), indicatorColor: Colors.purple.shade700,
  ...
  Widget _buildRollOnTab() {
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Icon(Icons.shield, color: F.h(Colors.purple, 700)),
        const SizedBox(width: 8),
        Flexible(child: Text('Ausgewähltes Deo', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: F.h(Colors.purple, 800)), overflow: TextOverflow.ellipsis)),
        const Spacer(),
      ...
      if (_selected != null)
        ...
        Container(width: double.infinity, padding: const EdgeInsets.all(32), decoration: BoxDecoration(color: F.h(Colors.grey, 50), borderRadius: BorderRadius.circular(12), border: Border.all(color: F.h(Colors.grey, 300))),
          child: Column(children: [
            Icon(Icons.shield, size: 48, color: F.h(Colors.grey, 300)), const SizedBox(height: 12),
            Text('Kein Deo ausgewählt', style: TextStyle(fontSize: 15, color: F.h(Colors.grey, 500))),
            const SizedBox(height: 8),
            Text('Tippen Sie auf "Auswählen" um ein Roll-on Deo zuzuweisen', style: TextStyle(fontSize: 12, color: F.h(Colors.grey, 400))),
          ])),
    ]));
  }
  ...
  Widget _buildStars(double rating) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...List.generate(5, (i) {
        if (i < rating.floor()) return Icon(Icons.star, size: 16, color: F.h(Colors.amber, 600));
        if (i < rating) return Icon(Icons.star_half, size: 16, color: F.h(Colors.amber, 600));
        return Icon(Icons.star_border, size: 16, color: Colors.amber.shade300);
      }),
      const SizedBox(width: 4),
      Text(rating.toStringAsFixed(1), style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: F.h(Colors.amber, 800))),
    ]);
  }
  ...
  Widget _buildProductCard(Map<String, dynamic> p) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: F.flaeche, borderRadius: BorderRadius.circular(12), border: Border.all(color: F.h(Colors.purple, 300), width: 2),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Text('${p['menge'] ?? ''}', style: TextStyle(fontSize: 12, color: F.h(Colors.grey, 600))),
            const SizedBox(width: 8),
            Text('${p['preis'] ?? ''}', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: F.h(Colors.brown, 700))),
          ]),
          ...
          if (besonderheiten.isNotEmpty) ...[
            Wrap(spacing: 6, runSpacing: 4, children: besonderheiten.map((h) => Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check_circle, size: 12, color: F.h(Colors.purple, 600)), const SizedBox(width: 3),
                Text(h.trim(), style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: F.h(Colors.purple, 800)))]),
            )).toList()),
            const SizedBox(height: 10),
          ],
          Text(p['beschreibung']?.toString() ?? '', style: TextStyle(fontSize: 12, color: F.h(Colors.grey, 700), height: 1.4)),
          if ((p['bezugsquelle']?.toString() ?? '').isNotEmpty) ...[
            const SizedBox(height: 8),
            Row(children: [Icon(Icons.shopping_cart, size: 12, color: F.h(Colors.grey, 500)), const SizedBox(width: 4),
              Text(p['bezugsquelle'].toString(), style: TextStyle(fontSize: 10, color: F.h(Colors.grey, 500)))]),
          ],
          const SizedBox(height: 10),
          Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(child: Text.rich(TextSpan(style: TextStyle(fontSize: 10, color: F.h(Colors.blue, 800), height: 1.4), children: [
                const TextSpan(text: 'Bewertung basiert auf 3 Kriterien:\n\n'),
                TextSpan(text: '1. Tests: ', style: TextStyle(fontWeight: FontWeight.bold, color: F.h(Colors.blue, 900))),
                ...
                TextSpan(text: '2. Kundenfeedback: ', style: TextStyle(fontWeight: FontWeight.bold, color: F.h(Colors.blue, 900))),
                ...
                TextSpan(text: '3. Praxistest: ', style: TextStyle(fontWeight: FontWeight.bold, color: F.h(Colors.blue, 900))),
                const TextSpan(text: 'ICD360S e.V. Mitglieder-Erfahrungen.\n\n'),
                TextSpan(text: '⚠ Hinweis: ', style: TextStyle(fontWeight: FontWeight.bold, color: F.h(Colors.orange, 800))),
                TextSpan(text: 'Diese Empfehlungen stellen keine medizinische oder gesundheitliche Beratung dar. Bei gesundheitlichen Beschwerden konsultieren Sie bitte Ihren Arzt oder Apotheker.', style: TextStyle(color: F.h(Colors.orange, 800))),
              ]))),
            ]),
          ),
        ])),
  ...
  void _showSelect() async {
    ...
    final selected = await showDialog<Map<String, dynamic>>(context: context, builder: (ctx) {
      ...
      return StatefulBuilder(builder: (_, setDlg) {
        ...
        return AlertDialog(
          title: Row(children: [Icon(Icons.shield, size: 18, color: F.h(Colors.purple, 700)), const SizedBox(width: 8), const Flexible(child: Text('Roll-on Deo auswählen', style: TextStyle(fontSize: 15), overflow: TextOverflow.ellipsis))]),
          content: SizedBox(width: 500, height: 450, child: Column(children: [
            ...
            Expanded(child: ListView.builder(itemCount: filtered.length, itemBuilder: (_, i) {
              ...
              return Card(color: isSel ? F.h(Colors.purple, 50) : null, child: ListTile(
                onTap: () => Navigator.pop(ctx, p),
                leading: CircleAvatar(backgroundColor: isSel ? F.h(Colors.purple, 100) : F.h(Colors.brown, 50),
                  child: Icon(Icons.shield, color: isSel ? F.h(Colors.purple, 700) : F.h(Colors.brown, 600), size: 18)),
                title: Text('${p['marke']} — ${p['name']}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: isSel ? F.h(Colors.purple, 800) : null)),
                subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...List.generate(5, (j) {
                      final bew = double.tryParse(p['bewertung']?.toString() ?? '0') ?? 0;
                      if (j < bew.floor()) return Icon(Icons.star, size: 12, color: F.h(Colors.amber, 600));
                      if (j < bew) return Icon(Icons.star_half, size: 12, color: F.h(Colors.amber, 600));
                      return Icon(Icons.star_border, size: 12, color: Colors.amber.shade300);
                    }),
                    const SizedBox(width: 3),
                    Text(p['bewertung']?.toString() ?? '', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: F.h(Colors.amber, 800))),
                  ]),
                  Row(children: [
                    Text('${p['menge']} • ${p['preis']}', style: const TextStyle(fontSize: 10)),
                    const SizedBox(width: 6),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1), decoration: BoxDecoration(color: F.h(Colors.purple, 50), borderRadius: BorderRadius.circular(4)),
                      child: Text(p['schutz']?.toString() ?? '', style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: F.h(Colors.purple, 700)))),
                  ]),
                ]),
                trailing: isSel ? Icon(Icons.check_circle, color: F.h(Colors.purple, 600), size: 18) : null,
              ));
            })),
          ])),
