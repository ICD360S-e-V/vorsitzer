...
class JasminaScreen extends StatefulWidget {
  ...
  @override
  State<JasminaScreen> createState() => _JasminaScreenState();
}

class _JasminaScreenState extends State<JasminaScreen> {
  ...
  String _website = 'https://www.joinjasmina.com';

  List<Map<String, dynamic>> _aufgaben = [];
  List<Map<String, dynamic>> _notizen = [];

  static const String _platform = 'jasmina';
  ...
  Future<void> _loadCredentials() async {
    try {
      final result = await widget.apiService.getPlatformCredentials(_platform);
      if (result['success'] == true && result['credentials'] != null) {
        final creds = result['credentials'];
        _emailController.text = creds['email'] ?? '';
        _passwordController.text = creds['password'] ?? '';
        _website = creds['website'] ?? 'https://www.joinjasmina.com';
      }
    } catch (_) {}
    ...
    if (_emailController.text.isEmpty && _passwordController.text.isEmpty) {
      try {
        ...
        if ((localEmail != null && localEmail.isNotEmpty) ||
            (localPassword != null && localPassword.isNotEmpty)) {
          _emailController.text = localEmail ?? '';
          _passwordController.text = localPassword ?? '';
          if (localUrl != null && localUrl.isNotEmpty) _website = localUrl;

          // Auto-save to server so migration is permanent
          await widget.apiService.savePlatformCredentials(
            platform: _platform,
          ...
          // Clean up old local keys
          await storage.delete(key: 'jasmina_login_url');
          await storage.delete(key: 'jasmina_login_email');
          await storage.delete(key: 'jasmina_login_password');

          debugPrint('[Jasmina] Migrated credentials from local storage to server');
        }
      } catch (e) {
        debugPrint('[Jasmina] Migration error: $e');
      }
    }
  }
  ...
  Future<void> _loadAufgaben() async {
    try {
      final result = await widget.apiService.getPlatformAufgaben(_platform);
  ...
  Future<void> _loadNotizen() async {
    try {
      final result = await widget.apiService.getPlatformNotizen(_platform);
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => WebViewScreen(
          title: 'Jasmina UG',
          url: _website,
          autoFillUsername: _emailController.text.isNotEmpty ? _emailController.text : null,
          autoFillPassword: _passwordController.text.isNotEmpty ? _passwordController.text : null,
        ),
      ),
    );
  }
  ...
  Future<void> _toggleAufgabe(Map<String, dynamic> aufgabe) async {
    final newErledigt = !(aufgabe['erledigt'] == true || aufgabe['erledigt'] == 1);
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildInfoRow(Icons.location_on, 'Adresse', 'Wintrichring 44, 80992 Munich, Germany'),
                                _buildInfoRow(Icons.person, 'Geschäftsführer', 'Mohamed Mezghanni'),
                                _buildInfoRow(Icons.email, 'E-Mail', 'info@joinjasmina.com'),
                                _buildInfoRow(Icons.phone, 'Telefon', '+49 (0)89 37040158'),
                                const Divider(height: 24),
                                _buildInfoRow(Icons.account_balance, 'Handelsregister', 'Amtsgericht München'),
                                _buildInfoRow(Icons.tag, 'Registernummer', 'HRB 305661'),
                                _buildInfoRow(Icons.place, 'Sitz', 'München, Deutschland'),
                                const Divider(height: 24),
                                const Text(
                                  'Rechtliches',
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    if (_aufgaben.where((a) => !(a['erledigt'] == true || a['erledigt'] == 1)).isNotEmpty)
                                      Container(
                                ...
                                if (_aufgaben.isEmpty)
                                  ...
                                  ...List.generate(_aufgaben.length, (i) {
                                    ...
                                    final erledigt = aufgabe['erledigt'] == true || aufgabe['erledigt'] == 1;
