...
class ApiService {
  ...
  Future<Map<String, dynamic>> updateUser({
    ...
    final response = await _client.post(
      ...
      body: jsonEncode({
        ...
        if (geburtsname != null) 'geburtsname': geburtsname,
        if (staatsangehoerigkeit != null) 'staatsangehoerigkeit': staatsangehoerigkeit,
        if (aufenthaltsstatus != null) 'aufenthaltsstatus': aufenthaltsstatus,
  ...
  // Gericht Datenbank
  Future<Map<String, dynamic>> getGerichtDatenbank(String gerichtTyp) async {
    final response = await _client.post(Uri.parse('$baseUrl/admin/gericht_datenbank.php'), headers: _headers,
      body: jsonEncode({'action': 'list', 'gericht_typ': gerichtTyp})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }

  // Telekom
  Future<Map<String, dynamic>> telekomAction(Map<String, dynamic> data) async {
    final response = await _client.post(Uri.parse('$baseUrl/vereinverwaltung/telekom_manage.php'), headers: _headers,
  ...
  // ========== WOHNGELDSTELLE (dedicated DB tables) ==========

  /// Shared Wohngeldstellen-Datenbank (Ämter): suchen / anlegen.
  Future<Map<String, dynamic>> searchWohngeldstellen({String search = '', String bundesland = ''}) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_manage.php'), headers: _headers, body: jsonEncode({'action': 'search', 'search': search, 'bundesland': bundesland})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  }
  /// Per-user Wohngeldstelle Stammdaten (ausgewähltes Amt, Aktenzeichen, Sachbearbeiter, Notizen).
  Future<Map<String, dynamic>> getWohngeldstelleData(int userId) async {
    final r = await _client.get(Uri.parse('$baseUrl/admin/wohngeldstelle_data_manage.php?user_id=$userId'), headers: _headers).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> saveWohngeldstelleData(int userId, Map<String, dynamic> data) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_data_manage.php'), headers: _headers, body: jsonEncode({'user_id': userId, 'data': data})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  // Anträge
  Future<Map<String, dynamic>> listWohngeldstelleAntraege(int userId) async {
    final r = await _client.get(Uri.parse('$baseUrl/admin/wohngeldstelle_antraege_manage.php?user_id=$userId'), headers: _headers).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> saveWohngeldstelleAntrag(int userId, Map<String, dynamic> data) async {
    final body = Map<String, dynamic>.from(data); body['user_id'] = userId;
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_antraege_manage.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> deleteWohngeldstelleAntrag(int id) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_antraege_manage.php'), headers: _headers, body: jsonEncode({'action': 'delete', 'id': id})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  // Antrag → Unterlagen (verschlüsselte BLOB-Dokumente)
  Future<Map<String, dynamic>> listWgAntragDocs(int antragId) async {
    final r = await _client.get(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php?antrag_id=$antragId&type=docs'), headers: _headers).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> uploadWgAntragDoc({required int antragId, required String filePath, required String fileName}) async {
    final uri = Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php');
    final request = http.MultipartRequest('POST', uri); request.headers.addAll(_headers);
    request.fields['antrag_id'] = antragId.toString(); request.fields['type'] = 'upload_doc';
    request.files.add(await http.MultipartFile.fromPath('file', filePath, filename: fileName));
    final sr = await request.send(); final response = await http.Response.fromStream(sr);
  ...
  Future<Map<String, dynamic>> deleteWgAntragDoc(int id) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php'), headers: _headers, body: jsonEncode({'action': 'delete', 'type': 'docs', 'id': id})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<http.Response> downloadWgAntragDoc(int id) async {
    return await _client.get(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php?type=download&id=$id'), headers: _headers).timeout(const Duration(seconds: 30));
  }
  // Antrag → Korrespondenz
  Future<Map<String, dynamic>> listWgAntragKorr(int antragId) async {
    final r = await _client.get(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php?antrag_id=$antragId&type=korr'), headers: _headers).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> saveWgAntragKorr(int antragId, Map<String, dynamic> data) async {
    final body = Map<String, dynamic>.from(data); body['antrag_id'] = antragId; body['type'] = 'korr';
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> deleteWgAntragKorr(int id) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/wohngeldstelle_antrag_detail.php'), headers: _headers, body: jsonEncode({'action': 'delete', 'type': 'korr', 'id': id})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  ...
  Future<Map<String, dynamic>> attachSozialamtAntragDocFromCloud({required int antragId, required int cloudFileId, required String docTyp}) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/sozialamt_antrag_docs.php'), headers: _headers,
  ...
  // ========== LANDRATSAMT VORFALL (eigene Tabelle, per-Spalten-Encryption) ==========

  Future<Map<String, dynamic>> listLandratsamtVorfaelle(int userId) async {
    final r = await _client.get(
      Uri.parse('$baseUrl/admin/landratsamt_vorfaelle_manage.php?user_id=$userId'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> saveLandratsamtVorfall(int userId, Map<String, dynamic> data) async {
    final body = Map<String, dynamic>.from(data); body['user_id'] = userId;
    final r = await _client.post(
      Uri.parse('$baseUrl/admin/landratsamt_vorfaelle_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> deleteLandratsamtVorfall(int id) async {
    final r = await _client.post(
      Uri.parse('$baseUrl/admin/landratsamt_vorfaelle_manage.php'),
      headers: _headers,
      body: jsonEncode({'action': 'delete', 'id': id}),
  ...
  /// Hängt Dateien an einen Bußgeld-Vorgang, wahlweise an ein bestimmtes
  /// Schreiben.
  ///
  /// ⚠️ Das Ergebnis wird zurückgegeben und MUSS ausgewertet werden. Bei den
  /// fünf `*_attachment.php`-Endpunkten warf der Client es weg — ein HTTP 500
  /// sah dort monatelang aus wie „nichts passiert": keine Meldung, keine
  /// Anhänge, kein Hinweis worauf.
  Future<Map<String, dynamic>> uploadBussgeldAnhaenge({
    required int userId,
    required int vorfallId,
    int? korrespondenzId,
    ...
    try {
      final req = http.MultipartRequest(
          'POST', Uri.parse('$baseUrl/admin/bussgeld_vorfall_upload.php'));
      // ⚠️ Erst kopieren, dann entfernen. `_headers` ist heute ein Getter,
      // der jedes Mal eine frische Map liefert — ein `..remove()` darauf
      // träfe also nur die Kopie. Würde daraus je ein Feld, löschte dieselbe
      // Zeile das Content-Type für ALLE Anfragen der App.
      ...
      if (korrespondenzId != null) req.fields['korrespondenz_id'] = '$korrespondenzId';
      req.fields['kategorie'] = kategorie;
      if (beschreibung != null && beschreibung.isNotEmpty) {
        req.fields['beschreibung'] = beschreibung;
      }
  ...
  /// Lädt einen Anhang. Die Datei liegt verschlüsselt; der Server entschlüsselt
  /// sie und prüft dabei die beim Hochladen gespeicherte Prüfsumme.
  Future<http.Response> downloadBussgeldAnhang(int userId, int dokumentId) async {
    return await _client.get(
      Uri.parse('$baseUrl/admin/bussgeld_vorfall_dok.php?id=$dokumentId&user_id=$userId'),
      headers: _headers,
    ).timeout(const Duration(seconds: 60));
  }

  Future<Map<String, dynamic>> deleteBussgeldAnhang(int userId, int dokumentId) async {
    try {
      final r = await _client.post(
        Uri.parse('$baseUrl/admin/bussgeld_vorfall_dok.php'),
        headers: _headers,
  ...
  /// %PDF-magic verified) or null on error.
  ///
  ...
  // (downloadVermieterDokument lives further down at line ~7133 — see
  //  there. Re-uses {dokumentId, userId} pattern as the existing call
  //  sites in the Vermieter tab.)
  ...
  Future<Map<String, dynamic>> paypalAction(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/vereinverwaltung/paypal_manage.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 15));
    try {
  ...
  Future<Map<String, dynamic>> simplefaxAction(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/vereinverwaltung/simplefax_manage.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 15));
    try {
  ...
  /// Generiert die WBA-Ausfüllhilfe (Begleitdokument) in der Sprache des Mitglieds —
  /// abgeleitet aus users.staatsangehoerigkeit (ukrainisch→uk, russisch→ru,
  /// rumänisch→ro, sonst→en). Übersetzt die Fragen des deutschen Antrags + zeigt die
  /// vorbelegten Antworten; eingereicht wird nur der deutsche Antrag. Gibt PDF-Bytes +
  /// Sprachcode zurück, oder null wenn keine Ausfüllhilfe nötig ist (deutsche
  /// Mitglieder → HTTP 204) bzw. bei Fehler.
  Future<({List<int> bytes, String lang})?> generateWbaAusfuellhilfePdf({required int userId, int? antragId}) async {
    try {
      final qp = <String, String>{'user_id': userId.toString()};
      if (antragId != null && antragId > 0) qp['antrag_id'] = antragId.toString();
      final r = await _client.get(
        Uri.parse('$baseUrl/admin/wba_ausfuellhilfe_pdf.php').replace(queryParameters: qp),
        headers: _headers,
      ).timeout(const Duration(seconds: 60));
      if (r.statusCode == 200 && r.bodyBytes.length > 1000 &&
          r.bodyBytes[0] == 0x25 && r.bodyBytes[1] == 0x50 && r.bodyBytes[2] == 0x44 && r.bodyBytes[3] == 0x46) {
        return (bytes: r.bodyBytes, lang: r.headers['x-wba-lang'] ?? '');
      }
    } catch (_) {}
    return null;
  }
  ...
  Future<Map<String, dynamic>> setUserGabelstaplerschein(int userId, bool value) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/user_qualifikationen.php'),
      headers: _headers,
      body: jsonEncode({'user_id': userId, 'action': 'set_gabelstapler', 'value': value ? 1 : 0}),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }
  ...
  Future<Map<String, dynamic>> vermieterAction(int userId, Map<String, dynamic> body) async {
    body['user_id'] = userId;
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/vermieter_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> getSanitaetshausData(int userId) async {
    final response = await _client.get(
      Uri.parse('$baseUrl/admin/sanitaetshaus_manage.php?user_id=$userId&action=all'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> searchSanitaetshausDatenbank(String q) async {
    final response = await _client.get(
      Uri.parse('$baseUrl/admin/sanitaetshaus_manage.php?action=datenbank&q=${Uri.encodeComponent(q)}'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> getSanitaetshausVorfallDetail(int userId, int vorfallId) async {
    final response = await _client.get(
      Uri.parse('$baseUrl/admin/sanitaetshaus_manage.php?user_id=$userId&vorfall_id=$vorfallId&action=vorfall_detail'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> sanitaetshausAction(int userId, Map<String, dynamic> body) async {
    body['user_id'] = userId;
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/sanitaetshaus_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> getJobcenterData(int userId) async {
    final response = await _client.get(
      Uri.parse('$baseUrl/admin/jobcenter_manage.php?user_id=$userId&action=all'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> searchJobcenterDatenbank(String q) async {
    final response = await _client.get(
      Uri.parse('$baseUrl/admin/jobcenter_manage.php?action=datenbank&q=${Uri.encodeComponent(q)}'),
      headers: _headers,
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  }
  ...
  Future<Map<String, dynamic>> jobcenterAction(int userId, Map<String, dynamic> body) async {
    body['user_id'] = userId;
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/jobcenter_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  // ===== Augenarzt-eigene Schweigepflicht (augenarzt_schweigepflicht* Tabellen/Endpunkte) =====
  Future<Map<String, dynamic>> augenarztCreateSchweigepflicht(Map<String, dynamic> payload) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_create.php'), headers: _headers, body: jsonEncode(payload)).timeout(const Duration(seconds: 30));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> augenarztSchweigepflichtAction(Map<String, dynamic> body) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_manage.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<http.Response> augenarztDownloadSchweigepflichtPdf(int id, {String type = 'pdf'}) async {
    return await _client.get(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_pdf.php?id=$id&type=$type'), headers: _headers).timeout(const Duration(seconds: 30));
  }
  Future<Map<String, dynamic>> augenarztUploadSchweigepflichtSignature({required int schweigepflichtId, required String type, required Uint8List bytes, required String filename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_signature_upload.php'));
    req.headers.addAll(_headers);
    req.fields['schweigepflicht_id'] = schweigepflichtId.toString();
    req.fields['type'] = type;
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
  ...
  Future<Map<String, dynamic>> augenarztDeleteSchweigepflichtSignature({required int schweigepflichtId, required String type}) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_signature_upload.php'), headers: _headers, body: jsonEncode({'schweigepflicht_id': schweigepflichtId, 'type': type, 'delete': true})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }
  Future<Map<String, dynamic>> augenarztDeleteSchweigepflichtSignatureById({required int signatureId}) async {
    return await augenarztSchweigepflichtAction({'action': 'delete_signature', 'signature_id': signatureId});
  }
  Future<http.Response> augenarztDownloadSchweigepflichtSignatureFile(int signatureId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_pdf.php?type=signature_file&signature_id=$signatureId'), headers: _headers).timeout(const Duration(seconds: 30));
  }
  Future<Map<String, dynamic>> augenarztCreateSchweigepflichtVersand({required int schweigepflichtId, required String methode, String? datum, String? faxNummer, String? emailAdresse, String? notiz, Uint8List? confirmationBytes, String? confirmationFilename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_versand_create.php'));
    req.headers.addAll(_headers);
    req.fields['schweigepflicht_id'] = schweigepflichtId.toString();
    req.fields['versand_methode'] = methode;
    if (datum != null && datum.isNotEmpty) req.fields['versand_datum'] = datum;
  ...
  Future<Map<String, dynamic>> augenarztDeleteSchweigepflichtVersand({required int versandId}) async {
    return await augenarztSchweigepflichtAction({'action': 'delete_versand', 'versand_id': versandId});
  }
  Future<http.Response> augenarztDownloadSchweigepflichtVersandConfirmation(int versandId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/augenarzt_schweigepflicht_pdf.php?type=versand_confirmation&versand_id=$versandId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  // === ARZT-VOLLMACHT (Vollmacht fuer Arzt/Apotheke — separat von Schweigepflicht
  //     und Behoerden-Vollmacht. Vollmacht erlaubt dem Verein, im Namen des
  //     Patienten Termine zu machen, Rezepte/Ueberweisungen abzuholen,
  //     E-Rezepte einzuloesen, etc. § 164 ff. BGB + § 360 SGB V) ===
  Future<Map<String, dynamic>> createArztVollmacht(Map<String, dynamic> payload) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/arzt_vollmacht_create.php'),
      headers: _headers,
      body: jsonEncode(payload),
    ).timeout(const Duration(seconds: 30));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> arztVollmachtAction(Map<String, dynamic> body) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/arzt_vollmacht_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<http.Response> downloadArztVollmachtPdf(int id, {String type = 'pdf'}) async {
    return await _client.get(
      Uri.parse('$baseUrl/admin/arzt_vollmacht_pdf.php?id=$id&type=$type'),
      headers: _headers,
    ).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> uploadArztVollmachtSignature({
    required int vollmachtId,
    required String type,
    required Uint8List bytes,
    required String filename,
  }) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/arzt_vollmacht_signature_upload.php'));
    req.headers.addAll(_headers);
    req.fields['vollmacht_id'] = vollmachtId.toString();
    req.fields['type'] = type;
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
  ...
  Future<Map<String, dynamic>> deleteArztVollmachtSignatureById({required int signatureId}) async {
    return await arztVollmachtAction({'action': 'delete_signature', 'signature_id': signatureId});
  }

  Future<http.Response> downloadArztVollmachtSignatureFile(int signatureId) async {
    return await _client.get(
      Uri.parse('$baseUrl/admin/arzt_vollmacht_pdf.php?type=signature_file&signature_id=$signatureId'),
      headers: _headers,
    ).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> createArztVollmachtVersand({
    required int vollmachtId,
    required String methode,
    String? datum,
    String? faxNummer,
    ...
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/arzt_vollmacht_versand_create.php'));
    req.headers.addAll(_headers);
    req.fields['vollmacht_id'] = vollmachtId.toString();
    req.fields['versand_methode'] = methode;
    if (datum != null && datum.isNotEmpty) req.fields['versand_datum'] = datum;
  ...
  Future<Map<String, dynamic>> deleteArztVollmachtVersand({required int versandId}) async {
    return await arztVollmachtAction({'action': 'delete_versand', 'versand_id': versandId});
  }

  Future<http.Response> downloadArztVollmachtVersandConfirmation(int versandId) async {
    return await _client.get(
      Uri.parse('$baseUrl/admin/arzt_vollmacht_pdf.php?type=versand_confirmation&versand_id=$versandId'),
      headers: _headers,
    ).timeout(const Duration(seconds: 30));
  }
  ...
  // ===================================================================
  // Krankenhaus — eigene Endpunkte (krankenhaus_*), Klon der Augenarzt-Methoden
  // ===================================================================
  Future<Map<String, dynamic>> krankenhausListKorrAttachments(String modul, int korrespondenzId) async {
    final r = await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_attachment.php?modul=$modul&korrespondenz_id=$korrespondenzId'), headers: _headers).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> krankenhausUploadKorrAttachment({required String modul, required int korrespondenzId, required String filePath, required String fileName}) async {
    final request = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_attachment.php')); request.headers.addAll(_headers);
    request.fields['modul'] = modul; request.fields['korrespondenz_id'] = korrespondenzId.toString();
    request.files.add(await http.MultipartFile.fromPath('file', filePath, filename: fileName));
    final response = await http.Response.fromStream(await request.send());
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> krankenhausDeleteKorrAttachment(int id) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_attachment.php'), headers: _headers, body: jsonEncode({'action': 'delete', 'id': id})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<http.Response> krankenhausDownloadKorrAttachment(int id) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_attachment.php?download_id=$id'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> getKrankenhausInstances(int userId) async {
    final r = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_get.php'),
      headers: _headers,
      body: jsonEncode({'user_id': userId}),
    ).timeout(const Duration(seconds: 20));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  ...
  Future<Map<String, dynamic>> saveKrankenhausInstance(int userId, int instance, Map<String, dynamic> data) async {
    final r = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_save.php'),
      headers: _headers,
      body: jsonEncode({'user_id': userId, 'instance': instance, 'data': data}),
    ).timeout(const Duration(seconds: 20));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  ...
  Future<Map<String, dynamic>> deleteKrankenhausInstance(int userId, int instance) async {
    final r = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_save.php'),
      headers: _headers,
      body: jsonEncode({'user_id': userId, 'instance': instance, 'action': 'delete'}),
  ...
  Future<Map<String, dynamic>> krankenhausRechnungAction(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_rechnung_manage.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> krankenhausRezeptAction(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_hilfsmittel_manage.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 20));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> krankenhausUploadGesundheitDokument({
    required int userId, required String gesundheitType, required String analyseId, required String filePath, required String fileName,
  }) async {
    final request = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_doc_upload.php'));
    final headers = Map<String, String>.from(_headers); headers.remove('Content-Type');
    request.headers.addAll(headers);
    request.fields['user_id'] = userId.toString();
    request.fields['gesundheit_type'] = gesundheitType;
  ...
  Future<http.Response> krankenhausDownloadGesundheitDokument(int docId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_doc_download.php?id=$docId'), headers: _headers).timeout(const Duration(seconds: 15));
  }

  Future<Map<String, dynamic>> krankenhausDeleteGesundheitDokument(int docId) async {
    final request = http.Request('DELETE', Uri.parse('$baseUrl/admin/krankenhaus_doc_delete.php'));
    request.headers.addAll(_headers);
    request.body = jsonEncode({'id': docId});
    final streamed = await _client.send(request).timeout(const Duration(seconds: 30));
    return jsonDecode(await streamed.stream.bytesToString());
  ...
  Future<Map<String, dynamic>> krankenhausListGesundheitDokumente(int userId, String gesundheitType, String analyseId) async {
    final response = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_doc_list.php'), headers: _headers, body: jsonEncode({'user_id': userId, 'gesundheit_type': gesundheitType, 'analyse_id': analyseId})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  }

  Future<Map<String, dynamic>> getKrankenhausTermine(int userId, String arztType) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_termine_list.php'),
      headers: _headers,
      body: jsonEncode({'user_id': userId, 'arzt_type': arztType}),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  ...
  Future<Map<String, dynamic>> saveKrankenhausTermin(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_termine_save.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  ...
  Future<Map<String, dynamic>> saveKrankenhausMedikament(Map<String, dynamic> data) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_medikamente_save.php'),
      headers: _headers,
      body: jsonEncode(data),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  ...
  Future<Map<String, dynamic>> uploadKrankenhausDoc({
    required int userId, required String gesundheitType, required String analyseId, required String filePath, required String fileName,
  }) async {
    final request = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_doc_upload.php'));
    final h = Map<String, String>.from(_headers); h.remove('Content-Type');
  ...
  Future<Map<String, dynamic>> listKrankenhausDocs({required int userId, required String gesundheitType, required String analyseId}) async {
    final response = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_doc_list.php'), headers: _headers, body: jsonEncode({'user_id': userId, 'gesundheit_type': gesundheitType, 'analyse_id': analyseId})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false, 'message': 'Invalid server response'}; }
  }

  Future<List<int>?> downloadKrankenhausDoc(int docId) async {
    final response = await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_doc_download.php?id=$docId'), headers: _headers).timeout(const Duration(seconds: 15));
    if (response.statusCode == 200 && response.headers['content-type'] != 'application/json') { return response.bodyBytes; }
    return null;
  }

  Future<Map<String, dynamic>> deleteKrankenhausDoc(int docId) async {
    final request = http.Request('DELETE', Uri.parse('$baseUrl/admin/krankenhaus_doc_delete.php'));
    request.headers.addAll(_headers);
    request.body = jsonEncode({'id': docId});
    final streamed = await _client.send(request).timeout(const Duration(seconds: 30));
  ...
  Future<Map<String, dynamic>> krankenhausCreateSchweigepflicht(Map<String, dynamic> payload) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_create.php'), headers: _headers, body: jsonEncode(payload)).timeout(const Duration(seconds: 30));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> krankenhausSchweigepflichtAction(Map<String, dynamic> body) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_manage.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<http.Response> krankenhausDownloadSchweigepflichtPdf(int id, {String type = 'pdf'}) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_pdf.php?id=$id&type=$type'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausUploadSchweigepflichtSignature({required int schweigepflichtId, required String type, required Uint8List bytes, required String filename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_signature_upload.php'));
    req.headers.addAll(_headers);
    req.fields['schweigepflicht_id'] = schweigepflichtId.toString();
    req.fields['type'] = type;
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
  ...
  Future<Map<String, dynamic>> krankenhausDeleteSchweigepflichtSignature({required int schweigepflichtId, required String type}) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_signature_upload.php'), headers: _headers, body: jsonEncode({'schweigepflicht_id': schweigepflichtId, 'type': type, 'delete': true})).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> krankenhausDeleteSchweigepflichtSignatureById({required int signatureId}) async {
    return await krankenhausSchweigepflichtAction({'action': 'delete_signature', 'signature_id': signatureId});
  }

  Future<http.Response> krankenhausDownloadSchweigepflichtSignatureFile(int signatureId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_pdf.php?type=signature_file&signature_id=$signatureId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausCreateSchweigepflichtVersand({required int schweigepflichtId, required String methode, String? datum, String? faxNummer, String? emailAdresse, String? notiz, Uint8List? confirmationBytes, String? confirmationFilename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_versand_create.php'));
    req.headers.addAll(_headers);
    req.fields['schweigepflicht_id'] = schweigepflichtId.toString();
    req.fields['versand_methode'] = methode;
    if (datum != null && datum.isNotEmpty) req.fields['versand_datum'] = datum;
  ...
  Future<Map<String, dynamic>> krankenhausDeleteSchweigepflichtVersand({required int versandId}) async {
    return await krankenhausSchweigepflichtAction({'action': 'delete_versand', 'versand_id': versandId});
  }

  Future<http.Response> krankenhausDownloadSchweigepflichtVersandConfirmation(int versandId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_schweigepflicht_pdf.php?type=versand_confirmation&versand_id=$versandId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausCreateArztVollmacht(Map<String, dynamic> payload) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_create.php'), headers: _headers, body: jsonEncode(payload)).timeout(const Duration(seconds: 30));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> krankenhausArztVollmachtAction(Map<String, dynamic> body) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_manage.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<http.Response> krankenhausDownloadArztVollmachtPdf(int id, {String type = 'pdf'}) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_pdf.php?id=$id&type=$type'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausUploadArztVollmachtSignature({required int vollmachtId, required String type, required Uint8List bytes, required String filename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_signature_upload.php'));
    req.headers.addAll(_headers);
    req.fields['vollmacht_id'] = vollmachtId.toString();
    req.fields['type'] = type;
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
  ...
  Future<Map<String, dynamic>> krankenhausDeleteArztVollmachtSignatureById({required int signatureId}) async {
    return await krankenhausArztVollmachtAction({'action': 'delete_signature', 'signature_id': signatureId});
  }

  Future<http.Response> krankenhausDownloadArztVollmachtSignatureFile(int signatureId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_pdf.php?type=signature_file&signature_id=$signatureId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausCreateArztVollmachtVersand({required int vollmachtId, required String methode, String? datum, String? faxNummer, String? emailAdresse, String? notiz, Uint8List? confirmationBytes, String? confirmationFilename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_versand_create.php'));
    req.headers.addAll(_headers);
    req.fields['vollmacht_id'] = vollmachtId.toString();
    req.fields['versand_methode'] = methode;
    if (datum != null && datum.isNotEmpty) req.fields['versand_datum'] = datum;
  ...
  Future<Map<String, dynamic>> krankenhausDeleteArztVollmachtVersand({required int versandId}) async {
    return await krankenhausArztVollmachtAction({'action': 'delete_versand', 'versand_id': versandId});
  }

  Future<http.Response> krankenhausDownloadArztVollmachtVersandConfirmation(int versandId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_vollmacht_pdf.php?type=versand_confirmation&versand_id=$versandId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausDmpAction(Map<String, dynamic> body) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_dmp_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 15));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> krankenhausCreateArztEinwilligung(Map<String, dynamic> payload) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_create.php'), headers: _headers, body: jsonEncode(payload)).timeout(const Duration(seconds: 30));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<Map<String, dynamic>> krankenhausArztEinwilligungAction(Map<String, dynamic> body) async {
    final r = await _client.post(Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_manage.php'), headers: _headers, body: jsonEncode(body)).timeout(const Duration(seconds: 15));
    try { return jsonDecode(r.body); } on FormatException { return {'success': false}; }
  }

  Future<http.Response> krankenhausDownloadArztEinwilligungPdf(int id, {String type = 'pdf'}) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_pdf.php?id=$id&type=$type'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausUploadArztEinwilligungSignature({required int einwilligungId, required String type, required Uint8List bytes, required String filename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_signature_upload.php'));
    req.headers.addAll(_headers);
    req.fields['einwilligung_id'] = einwilligungId.toString();
    req.fields['type'] = type;
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
  ...
  Future<Map<String, dynamic>> krankenhausDeleteArztEinwilligungSignatureById({required int signatureId}) async {
    return await krankenhausArztEinwilligungAction({'action': 'delete_signature', 'signature_id': signatureId});
  }

  Future<http.Response> krankenhausDownloadArztEinwilligungSignatureFile(int signatureId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_pdf.php?type=signature_file&signature_id=$signatureId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausCreateArztEinwilligungVersand({required int einwilligungId, required String methode, String? datum, String? faxNummer, String? emailAdresse, String? notiz, Uint8List? confirmationBytes, String? confirmationFilename}) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_versand_create.php'));
    req.headers.addAll(_headers);
    req.fields['einwilligung_id'] = einwilligungId.toString();
    req.fields['versand_methode'] = methode;
    if (datum != null && datum.isNotEmpty) req.fields['versand_datum'] = datum;
  ...
  Future<Map<String, dynamic>> krankenhausDeleteArztEinwilligungVersand({required int versandId}) async {
    return await krankenhausArztEinwilligungAction({'action': 'delete_versand', 'versand_id': versandId});
  }

  Future<http.Response> krankenhausDownloadArztEinwilligungVersandConfirmation(int versandId) async {
    return await _client.get(Uri.parse('$baseUrl/admin/krankenhaus_einwilligung_pdf.php?type=versand_confirmation&versand_id=$versandId'), headers: _headers).timeout(const Duration(seconds: 30));
  }

  Future<Map<String, dynamic>> krankenhausKorrespondenzAction(Map<String, dynamic> body) async {
    final response = await _client.post(
      Uri.parse('$baseUrl/admin/krankenhaus_korrespondenz_manage.php'),
      headers: _headers,
      body: jsonEncode(body),
    ).timeout(const Duration(seconds: 20));
    try { return jsonDecode(response.body); } on FormatException { return {'success': false}; }
  ...
  Future<Map<String, dynamic>> uploadKrankenhausKorrespondenzAnhang({
    required int korrespondenzId,
    required Uint8List bytes,
    required String filename,
  }) async {
    final req = http.MultipartRequest('POST', Uri.parse('$baseUrl/admin/krankenhaus_korrespondenz_anhang_upload.php'));
    req.headers.addAll(_headers);
    req.fields['korrespondenz_id'] = korrespondenzId.toString();
    req.files.add(http.MultipartFile.fromBytes('file', bytes, filename: filename));
    final stream = await req.send().timeout(const Duration(seconds: 60));
  ...
  Future<http.Response> downloadKrankenhausKorrespondenzAnhang(int anhangId) async {
    return await _client.get(
      Uri.parse('$baseUrl/admin/krankenhaus_korrespondenz_pdf.php?anhang_id=$anhangId'),
      headers: _headers,
    ).timeout(const Duration(seconds: 30));
  }
